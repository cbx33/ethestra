...
class Seq(gobject.GObject):
		...
		def PlayBar(self):
			if self.mute == 0:
				for note in self.pattern:
					...
					gobject.timeout_add(self.TimeForBeat(note[0]), self.PlayNote, note[1] + self.transpose + self.tran_mod, note[2], note[3])
	...
	def AddChannel(self, chan, name="Blank", modable=1):
		self.channels.append(seq.Channel(self.device, chan, self.tempo, name, modable))
